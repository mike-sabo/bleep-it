...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var toggleCleanWord = function () {
        ...
        if(wordEnabled == true) {
            chrome.storage.sync.set({'enableWords': 'True'}, function () {
            });
        } else {
            chrome.storage.sync.set({'enableWords': 'False'}, function () {
            });
        }
    };
    ...
    var updateUserAddedWords = function () {
        ...
        if(setWords == "") {
            if (confirm('Your user-added list appears to be empty, are you sure you want to erase all of your words?')) {
                chrome.storage.sync.set({'userAddedWords': null}, function(items) {
            ...
            chrome.storage.sync.set({'userAddedWords': setWords}, function(items) {
                chrome.notifications.create("", opt, function() {});
                getUserWords();
            }); 
        }     
